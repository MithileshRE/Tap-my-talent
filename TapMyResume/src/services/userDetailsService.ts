...
/**
 * Get user's master education data
 * @param email User's email
 * @returns User's education and certification data or throws error if not found
 */
export const getMasterEdu = async (email: string): Promise<EducationCertificationData> => {
  try {
    const response = await axios.get(`${BASE_URL}/userDetails/Master-Edu?email=${encodeURIComponent(email)}`, {
      headers: {
        accept: 'application/json',
      },
    });
    ...
    console.error('Failed to fetch master education data:', error);
    throw error;
  }
};
